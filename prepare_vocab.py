...
def parse_args():
    ...
    parser.add_argument('--min_freq', type=int, default=0, help='If > 0, use min_freq as the cutoff.') ## cutoff是什么？
...
def main():
    ...
    vocab_file = args.vocab_dir + '/vocab.pkl'   # 词表
    emb_file = args.vocab_dir + '/embedding.npy' # embedding [token_id, token_vec]
    ...
    print("building vocab...") # tokens 表 + 两个特殊 tokens + masks list
    v = build_vocab(train_tokens, glove_vocab, args.min_freq)

    print("calculating oov...") # 计算没有匹配的单词比例
    ...
    print("building embeddings...") # embedding [token_id, token_vec]
    embedding = vocab.build_embedding(wv_file, v, wv_dim) # glove, tokens 表
...
# tokens min_freq 或者 tokens in glove，+ 两个特殊token + masks
...
# 返回 tokens 的数量和没有匹配的 tokens 的数量
def count_oov(tokens, vocab):

    c = Counter(t for t in tokens) # Counter 是 dict 的子类，对可哈希对象进行高效计数， key 是可迭代的可哈希对象， value 是出现次数
    total = sum(c.values()) # tokens 的数量
    matched = sum(c[t] for t in vocab) # 匹配的 tokens 的数量
